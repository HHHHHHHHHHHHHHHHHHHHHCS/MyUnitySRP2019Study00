Shader "MyRP/Cloud/MeshCloud_Tessellation"
{
	...
	SubShader
	{
		Tags
		{
			"RenderType" = "Opaque" "Queue" = "AlphaTest" "IgnoreProjector" = "True"
		}
		LOD 100
		...
		Pass
		{
			...
			half4 frag(g2f i): SV_Target
			{
				...
				//太远就不要阴影了
				half shadow = saturate(lerp(mainLight.shadowAttenuation, 1.0,
				                            (distance(i.worldPos.xyz, _WorldSpaceCameraPos.xyz) - 100) * 0.1));
				half finalLit = saturate(
					smoothNdotV * 0.5 + shadow * saturate(smoothNdotL + backSSS) * (1 - NdotV * 0.5));
