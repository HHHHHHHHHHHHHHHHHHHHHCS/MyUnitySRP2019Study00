Shader "MyRP/Cloud/SolidCloud"
{
	...
	v2f DefaultVert(a2v v)
	{
		...
		#elif FRAME_MODE_3
		#if FRAME_BLEND
					o.pos.y = (1 - 2 * _Frame) * (o.pos.y + 1) ;
					o.uv.y = 1 - _Frame - o.pos.y;
		#else
		#if CLOUD_MUL_RT_ON
						o.uv.y = 0.5 * (_Frame + o.uv.y);
		#else
						o.pos.y = (1 - 2 * _Frame) * (o.pos.y + 1) ;
						o.uv.y =  0.5 - 0.5 * o.pos.y;
		#endif
		#endif
		#endif
		#endif
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma multi_compile_local FRAME_MODE_0 FRAME_MODE_2 FRAME_MODE_3 FRAME_MODE_4
			...
			#if CLOUD_FRAME_ON && CLOUD_MUL_RT_ON && FRAME_MODE_4
				float4 _MulRTScale; // xy:rt size    zw: step,scale
			#endif
			...
			{
				#if CLOUD_FRAME_ON && CLOUD_MUL_RT_ON && FRAME_MODE_4
						float2 uv = (0.5 + (((i.pos.xy - 0.5) + 0.5 * float2(_Frame >> 1, _Frame & 1)) * _MulRTScale.w + _MulRTScale.zz)) / _MulRTScale.xy;
				#else
				float2 uv = i.uv;
				#endif
				// return half4(uv, 0, 1);
		...
		Pass
		{
			...
			TEXTURE2D(_BlendTex);
			SAMPLER(sampler_clamp_linear_BlendTex);

			#if CLOUD_BLUR_ON
				float4 _Step_TexelSize;
			#endif

			half4 FragOut(v2f i): SV_TARGET
			{
				#if !CLOUD_BLUR_ON
				half4 col = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv, 0);
				return col;
				#else
				float2 step = _Step_TexelSize.xy;
				
				half4 col = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv, 0);

				half4 col1 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(step.x, 0), 0);
				half4 col2 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(-step.x, 0), 0);
				half4 col3 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(0, step.y), 0);
				half4 col4 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(0, -step.y), 0);

				// half4 col5 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(step.x, step.y), 0);
				// half4 col6 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(-step.x, step.y), 0);
				// half4 col7 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(step.x, -step.y), 0);
				// half4 col8 = SAMPLE_TEXTURE2D_LOD(_BlendTex, sampler_clamp_linear_BlendTex, i.uv + float2(-step.x, -step.y), 0);
				
				half a0 = min(min(col1.a, col2.a), min(col3.a, col4.a));
				// half a1 = min(min(col5.a, col6.a), min(col7.a, col8.a));
				half a = min(col.a, a0) > 0.0001 ? 1 : 0;
		...
		Pass
		{
			...
			half4 FragOut(v2f i): SV_TARGET
			{
				...
				#if CLOUD_FRAME_ON && CLOUD_MUL_RT_ON && FRAME_MODE_4
						const float2 pos = fmod(i.pos.xy, 2) - 0.5;
						const float2 discardPos = float2(_Frame >> 1, _Frame & 1);
						if (pos.x != discardPos.x || pos.y != discardPos.y)
						{
							discard;
						}
						half4 col = 0;
						col += 0.8 * LOAD_TEXTURE2D_LOD(_TempBlendTex1, floor((i.pos.xy - 0.5) / 2), 0);
						col += 0.2 * LOAD_TEXTURE2D_LOD(_TempBlendTex2, floor((i.pos.xy - 0.5) / 4), 0);
						return col;
				#else
				const float2 uv = i.uv;
				half4 col = 0;
				col += 0.8 * SAMPLE_TEXTURE2D_LOD(_TempBlendTex1, sampler_linear_clamp_TempBlendTex1, uv, 0);
				col += 0.2 * SAMPLE_TEXTURE2D_LOD(_TempBlendTex2, sampler_linear_clamp_TempBlendTex2, uv, 0);
				return col;
				#endif
			}
			ENDHLSL
