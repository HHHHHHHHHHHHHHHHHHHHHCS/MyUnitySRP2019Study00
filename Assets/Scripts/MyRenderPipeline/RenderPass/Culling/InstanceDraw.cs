...
public class InstanceDraw : MonoBehaviour
{
	...
	private void Start()
	{
		Camera.main.transform.position = new Vector3(-100f, 0.0f, 0.0f);
		Init();
	}
	...
	private void Init()
	{
		         ...
		         || rendererType == RendererType.DrawMeshInstancedIndirect)
		{
			...
			if (rendererType == RendererType.DrawMeshInstancedIndirect)
			{
				...
				matrixsBuffer = new ComputeBuffer(count, sizeof(float) * 16, ComputeBufferType.Structured);
