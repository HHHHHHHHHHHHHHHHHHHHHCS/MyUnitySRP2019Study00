...
namespace MyRenderPipeline.RenderPass.HiZIndirectRenderer.Builtin
{
	...
	public class HiZBuiltinBuffer : MonoBehaviour
	{
		// Enums
		private enum Pass
		{
		...
		public void Start()
		{
			...
			m_ShadowmapCopy =
				new RenderTexture(1024, 1024, 0, RenderTextureFormat.RGHalf, RenderTextureReadWrite.Linear);

			m_lightShadowCommandBuffer = new CommandBuffer();
			m_lightShadowCommandBuffer.name = "Copy Shadow";
		...
		public void InitializeTexture()
		{
			...
			m_HiZDepthTexture =
				new RenderTexture(size, size, 0, RenderTextureFormat.RGHalf, RenderTextureReadWrite.Linear);
		...
		private void OnPreRender()
		{
			...
			if (m_HiZDepthTexture == null
			    || (m_HiZDepthTexture.width != size || m_HiZDepthTexture.height != size)
			...
			if (m_CommandBuffer == null || isCommandBufferInvalid == true)
			{
				...
				{
					m_Temporaries[i] = Shader.PropertyToID("_LOD_Temporaries" + i.ToString());
					size >>= 1;
