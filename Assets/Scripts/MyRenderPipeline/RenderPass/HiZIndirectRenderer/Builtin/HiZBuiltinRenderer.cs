...
{
	public class HiZBuiltinRenderer : MonoBehaviour
	{
		...
		public HiZBuiltinBuffer hiZBuiltinBuffer;
		...
		public void Initialize(ref HiZBuiltinInstanceData[] instances)
		{
			if (!m_isInitialized)
			{
		...
		public void StopDrawing(bool shouldReleaseBuffers = false)
		{
			...
			if (shouldReleaseBuffers)
			{
				...
				hiZBuiltinBuffer.enabled = false;
			}
		}
		...
		/// <returns></returns>
		private bool InitializeRenderer(ref HiZBuiltinInstanceData[] _instances)
		{
			if (!TryGetKernels())
			{
				return false;
			...
			hiZBuiltinBuffer.enabled = true;
			hiZBuiltinBuffer.InitializeTexture();
			...
			for (int i = 0; i < m_numberOfInstanceTypes; i++)
			{
				...
				HiZBuiltinInstanceData iid = _instances[i];
			...
			occlusionCS.SetVector(_HiZTextureSize_ID, hiZBuiltinBuffer.TextureSize);
			occlusionCS.SetTexture(m_occlusionKernelID, _HiZMap_ID, hiZBuiltinBuffer.Texture);
