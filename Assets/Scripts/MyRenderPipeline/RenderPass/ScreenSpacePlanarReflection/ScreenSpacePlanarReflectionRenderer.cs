...
namespace MyRenderPipeline.RenderPass.ScreenSpacePlanarReflection
{
	...
	public class ScreenSpacePlanarReflectionRenderer : ScriptableRenderPass
	{
		...
		{
			RenderTextureDescriptor rtd = new RenderTextureDescriptor(RTWidth, RTHeight, RenderTextureFormat.Default, 0, 0);
			...
			if (settings.shouldRenderSSPR)
			{
				...
				if (ShouldUseSinglePassUnsafeAllowFlickeringDirectResolve())
				{
					...
					cb.SetComputeTextureParam(cs, kernel_MobilePathSinglePassColorRTDirectResolve, "PosWSyRT",
						sspr_PosWSyRT_rti);
					cb.SetComputeTextureParam(cs, kernel_MobilePathSinglePassColorRTDirectResolve, "_CameraOpaqueTexture",
						new RenderTargetIdentifier("_CameraOpaqueTexture"));
					cb.SetComputeTextureParam(cs, kernel_MobilePathSinglePassColorRTDirectResolve, "_CameraDepthTexture",
				...
				else
				{
					...
					cb.DispatchCompute(cs, kernel_NonMobilePathClear, dispatchThreadGroupXCount, dispatchThreadGroupYCount,
